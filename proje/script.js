...
// --- YENİ: AYT Sorularını HTML'e Ekleme Fonksiyonu ---
function generateAYTQuestions() {
    const questionsListDiv = document.getElementById('ayt-questions-list');
    if (!questionsListDiv) {
        console.error("AYT soru listesi div'i bulunamadı!");
        return;
    }

    let questionsHTML = ''; // Tüm soruların HTML'ini biriktir

    aytQuestionsData.forEach((qData, index) => {
        // Seçenekler için HTML oluştur
        let optionsHTML = '';
        const optionLetters = ['A', 'B', 'C', 'D', 'E'];
        qData.options.forEach((option, optIndex) => {
            optionsHTML += `<div>${optionLetters[optIndex]}) ${option}</div>`;
        });

        // Tek bir soru için HTML yapısı
        questionsHTML += `
            <div class="ayt-question">
                <div class="question">${index + 1}. ${qData.question}</div>
                <div class="options">
                    ${optionsHTML}
                </div>
                <button class="answer-btn" onclick="toggleAnswer(this)">Cevabı Göster</button>
                <div class="answer">
                    <strong>Doğru Cevap: ${qData.answerLetter}</strong><br>
                    ${qData.explanation}
                </div>
            </div>
        `;
    ...
    // Oluşturulan tüm HTML'i sayfaya ekle
    questionsListDiv.innerHTML = questionsHTML;
}
// --- generateAYTQuestions Fonksiyonu Sonu ---

// --- Ana Başlatma Fonksiyonu (init) ---
function init() {
    // ... (Diğer init kodları) ...
    ...
    // --- YENİ: AYT Sorularını Oluştur ---
    generateAYTQuestions(); // Soruları oluşturmak için fonksiyonu çağır

} // init fonksiyonu burada biter
